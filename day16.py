...
def day16(input):
    ...
    l = input.split("\n")
    regex = r"(?:(\w+): (\d+),?)+"
    traits = [[(k, int(v)) for k, v in findall(regex, line)] for line in l]
    for sue, ts in enumerate(traits):
      possible = True
      for key, value in ts:
        # Special case for trees and cats
        if (key == "trees" or key == "cats") and value <= tickerDict[key]:
          possible = False
          continue
            
        # Special case for pomeranians and goldfish
        if (key == "pomeranians" or key == "goldfish") and value >= tickerDict[key]:
          possible = False
          continue

        if (key not in ["pomeranians", "goldfish", "trees", "cats"]) and value != tickerDict[key]:
          possible = False
          continue
